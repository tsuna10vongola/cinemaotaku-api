...
app.delete('/:id', async (req, res) => {
    try {
        ...
        const responseAnime = {
            id: deletedAnime._id,
            title: deletedAnime.title,
            english: deletedAnime.english,
            description: deletedAnime.description,
            image: deletedAnime.image,
            trailer: deletedAnime.trailer,
            season: deletedAnime.season,
            dub: deletedAnime.dub,
            time: deletedAnime.time,
            genres: deletedAnime.genres,
            completed: deletedAnime.completed,
            studio: deletedAnime.studio
        };
...
const EpisodioSchema = new mongoose.Schema({
    _id: { type: Number, required: true }, // Adiciona a propriedade "id" para o número do episódio
    title: String,
    image: String,
    video: String,
    anime: {
        type: mongoose.Schema.Types.ObjectId,
        ref: 'Anime' // Nome do modelo de anime
    }
});
...
app.post('/:animeId/episodios', async (req, res) => {
    try {
        ...
        const { _id, title, image, video } = req.body;
...
app.put('/:animeId/episodios/:id', async (req, res) => {
    try {
        const episodio = await Episodio.findOneAndUpdate(
            { anime: req.params.animeId, _id: req.params.id },
            req.body,
            { new: true }
        );
